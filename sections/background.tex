...
Specifically, the behavior of an \ttt{ActiveClass} is described by a state machine\footnote{It is possible to have multiple state machines in the same active class. However, for simplification, this paper assumes that an active class only holds a state machine}.
...
In fact, the support of \ttt{Action} is convenient in many modeling tools such as IBM Rhapsody and Enterprise Architect, which allow the developers to write the low level code (user-code) directly at the model level.
...
%\begin{definition} 
	...
	A state with is \ttt{composite} if it contains at least one region; \ttt{orthogonal/concurrent} if contains more than one region; otherwise \ttt{simple}. 
	...
	A connection point is a vertex, whose kind $\in \{enpoint, expoint\}$, and contained by a composite state.%A concurrent state contains more than one region.
%\end{definition}		
...
The detailed semantics of each pseudo state is clearly defined in the UML specification \cite{OMG2015} and beyond the scope of this paper.
...
%\begin{definition} 
	A transition $t \in T$ is an edge connecting two vertexes, \ttt{source} and \ttt{target}. 
	%The source and target vertexes of an edge ed are obtained by $src(t)$ and $tgt(t)$. 
	A transition $t$ can have a guard $guard(t)$, an effect $effect(t)$, and is associated with a set of events. 
...
\begin{definition} An event is one of the followings:
	\begin{itemize}
		...
		\item A \ttt{ChangeEvent} is associated with a boolean expression \ttt{expr} written in C++. \ttt{ChangeEvent} is emitted if \ttt{expr} changes from true (false) to false (true).
