...
This paper describes \tb{XSeparation} - an code generation approach which enables the bidirectional traceability between architecture model and code. %allows to extremely separate architecture-generated code and user-code. 
%, which consists of structure and behavior models
Current programming language elements are at lower level of abstraction than software architectures.
To establish a bidirectional traceability between architecture model and code, the core idea of XSeparation is to leverage an object-oriented language by creating additional constructs for expressing architectural information.
The established traceability is then combined with our synchronization methodological pattern presented in \cite{foster2016} for synchronization.
