...
\item \textit{state machine} $\rightarrow$ \tf{'state\_machine('name') \{vertices\};'} 

\item \ti{state} $\rightarrow$ \tf{'state ('name, ent, ex');'} 

\item \ti{state} $\rightarrow$ \tf{'initial\_state ('name, ent, ex, effect');'}

\item \ti{concurrent state} $\rightarrow$ \tf{'state ('name, ent, ex') \{' regions '\};'} 
...
\item \ti{event queue size} $\rightarrow$ \tf{'configuration('}
\end{itemize}
...
\noindent
\tb{Example:}
Lines 2-10 in Fig. \ref{fig:frontend-overview} (a) represents the state \ttt{S1} of the USM example in Fig. \ref{fig:illustration} (a) and its vertices.
...
\tb{Syntax:}
\begin{itemize}[\footnotesize]
\item \ti{enpoint} $\rightarrow$ \tf{'entry\_point ('name');'} 

\item \ti{expoint} $\rightarrow$ \tf{'exit\_point ('name');'}
...
Furthermore, an action \ttt{effect1} is executed during the transition fire.
...
\item \ttt{transition(S11,S2,NULL,SE,effect2)}: A transition, triggered by the signal event \ttt{SE}, and executes an effect \ttt{effect2}.
\ttt{effect2} in Listing \ref{lst:effect-segment} has a parameter typed by the signal \ttt{Sig} associated with the event \ttt{SE}.
\end{itemize}
