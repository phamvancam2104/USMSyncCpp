...
Several tools \cite{ibm_rhapsody, Magicdraw} support code generation from UML models and reverse engineering using a bidirectional mapping between UML class diagrams and object-oriented code.
...
Separation \cite{steinberg2008emf} uses specialized comments such as \textsf{@generated NOT} to preserve code modified by programmers by separating the programmer-modified code from generated areas. 
...
\ttt{xMapper} \cite{zheng2012enhancing} overcomes limitations of the separation by separating generated and modified code in different programming constructs.
However, %as the authors state that, 
it does not allow to map code elements back to architecture elements.  
...
\noindent
\tb{Text-based modeling languages (TMLs):} TMLs such as Umple \cite{lethbridge2010umplification} and Earl Grey (EG) \cite{mazanec2012general} can have bidirectional mapping to UML elements, then reduce the perception gap between diagram-based and textual languages.
The difference to our approach is that we extend an existing programming language to connect to architecture while the TMLs are simply textual descriptions of UML models.
To efficiently use the TMLs, programmers are forced to change their working environment while our approach does not impose that.
Furthermore, the TMLs do not support full features of the aforementioned UML diagrams.
%Furthermore, 
...
The Boost library \cite{boost} also tries to build a bidirectional mapping between C++ and UML State Machine.
...
Our approach is inspired of ArchJava \cite{aldrich2002archjava}, which adds structural concepts such as part and port to Java. %to support the co-evolution of architecture structure and Java implementation. 
...
ArchJava makes it not Java anymore and facilities of IDEs such as auto-completion are not aware.%, XSeparation-generated code can use these facilities as discussed in \ref{sec:xseparationarchitecture}.
