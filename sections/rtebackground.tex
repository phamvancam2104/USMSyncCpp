...
%This section defines the actors who will use RAOES to collaborate during software development process.
%Then we define the main capabilities, as use-cases, expected from a generic IDE used by these actors.
%Some basic concepts related to code generation use-cases, which will be offered by RAOES, are also defined in this section.
...
%\begin{definition}
%	A \ttt{model-driven developer} (MDD) is an actor who uses the model as the main working artifact. \ttt{Code-driven developer} (CDD) is an actor who uses the code as the main working artifact.
%\end{definition}
...
%Software architects and programmers are specializations of the MDD and the CDD, respectively.
...
A developer (software architect or programmer) can either use \ttt{Generate Code (Batch)} or \ttt{Generate Code (Incremental)} related to forward engineering, and \ttt{Reverse Code (Batch)} or \ttt{Reverse Code (Incremental)}, related to reverse engineering. The definitions of these are as followings.
%\end{comment}

\begin{definition}\ttt{Batch code generation} \cite{Giese2006} is a process of generating code
	from a model, from scratch.
	Any existing code is overwritten by the newly generated code.
	In contrast, \ttt{Incremental code generation} takes as input an edited model and existing code to update the code by propagating
...
\begin{definition}%[Batch reverse engineering]
	...
	\ttt{Incremental reverse engineering} takes as
...
our process for model-code synchronization. %and is detailed in Section \ref{sec:collaboration}.
